...
if (fs.existsSync(envPath)) {
  for (const raw of fs.readFileSync(envPath, "utf8").split(/\r?\n/)) {
    ...
    const m = /^([\w.-]+)\s*=\s*(.*)$/.exec(s); if (!m) continue;
...
export async function one(q, p = []) { const { rows } = await pool.query(q, p); return rows[0] ?? null; }
export async function all(q, p = []) { const { rows } = await pool.query(q, p); return rows; }
export async function run(q, p = []) { return pool.query(q, p); }
